...
class graphcl(nn.Module):
    def __init__(self, gnn, node_imp_estimator):
        ...
        # Learnable parameters for Gaussian distribution
        self.mu = torch.nn.Parameter(torch.tensor(0.8))
        self.sigma = torch.nn.Parameter(torch.tensor(0.2))
    ...
    def forward_cl_auto(self, x, edge_index, edge_attr, batch):
        node_imp = self.node_imp_estimator(x, edge_index, edge_attr, batch)
        x = self.gnn(x, edge_index, edge_attr)
        ...
        # 标准正态分布采样
        epsilon = torch.rand(1).to(node_imp.device)
        
        # 高斯因子通过重参数化变换得到
        gaussian_factor = self.mu + epsilon * self.sigma

        node_imp *= gaussian_factor
        x = torch.mul(x, node_imp)
        x = self.pool(x, batch)
        x = self.projection_head(x)
        return x
