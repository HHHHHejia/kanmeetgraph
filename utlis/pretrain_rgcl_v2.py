...
class graphcl(nn.Module):
    ...
    def loss_ra(self, x1, x2, x4, 
        ...
        if (special_loss_flag == False):
            #D(f(x1,x4))
            feature_dis = self.cal_fea_dis(feature_sim)
            #d(x1, x4)
            cl_loss = 0.5*torch.norm(feature_dis - graph_dis, p = 2)

        else:
            graph_sim = 1 - graph_dis
            cl_loss = 0.5*torch.norm(feature_sim - graph_sim, p = 2)
...
def train(args, model, device, dataset, optimizer):
    ...
    dataset4.aug, dataset4.aug_ratio = args.aug4, args.aug_ratio4
    ...
    #特殊情况
    special_loss_flag = False
    # if (args.aug_ratio1 == args.aug_ratio4):
    #     special_loss_flag = True
    print("special_loss_flag:", special_loss_flag)
...
def main():
    ...
    parser.add_argument('--aug4', type=str, default='dropN')
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        train_loss, ra_loss, cl_loss = train(args, model, device, dataset, optimizer)
